...
On n'utilise pas partner pour le hashCode car c'est un attribut mutable qui change après l'insertion dans le HashSet (via matchWith/breakOff). Si hashCode() en dépend, un participant change de bucket une fois haché — le contrat Java est violé : l'objet devient introuvable avec contains()/remove() même s'il est toujours physiquement dans le set. 

On n'utilise pas le hashCode() par défaut d'Object car il est basé sur l'identité mémoire (implémentation typique : dérivé de l'adresse de l'objet). Deux Participant représentant la même personne (même nom) mais construits séparément seraient alors traités comme différents, et l'equals() par défaut (==) romprait la sémantique métier attendue — typiquement pour cheaters()/registration(), dont la logique de "même participant" est censée reposer sur l'identité logique (le nom), pas sur la référence objet.

A retenir : Si on veut que l'identité d'un objet dans un Set ou une Map repose sur un de ses attributs il faut un equals et un hashCode cohérent. Sinon, par défaut, c'est la référence mémoire qui sera utilisée.
